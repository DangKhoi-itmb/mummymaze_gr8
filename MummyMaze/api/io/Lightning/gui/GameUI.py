...
_player = None
_world_map = None
_text_designer = None # Dùng để vẽ điểm/giờ
...
def _reset_runtime_state():
    ...
    global _show_options, _steps_taken, _level_start_time, _current_score
    ...
    _show_options = False
    _steps_taken = 0
    _current_score = 1000 # Điểm cơ bản
...
def initialize_ui(mode="random", level_id=1, size=8, difficulty="medium"):
    ...
    global _snake_img, _mumlogo_img, _maze_size, _text_designer
    ...
    _text_designer = TextDesigner(color=(255, 255, 200)) # Màu vàng nhạt cho HUD
    ...
    try:
        ...
        print(f"⚠️ MazeLoader init failed: {e}")
        _maze_loader = MazeLoader(level_id=None, difficulty=difficulty, generate_infinite=True, maze_size=size)
...
def restart_level():
    """
    Logic Retry chuẩn:
    - Reset vị trí nhân vật/quái/cửa về ban đầu.
    - KHÔNG sinh lại mê cung mới (giữ nguyên seed/cấu trúc cũ).
    - Reset thời gian và bước đi.
    """
    ...
    # 2. Reset Maze Entities (nhưng giữ nguyên tường)
    if _maze_loader:
        _maze_loader.reset()
        p = _maze_loader.parsed["player"]
        _player = Player(p["x"], p["y"], _maze_loader.maze_size, _maze_loader.cell_size)
...
def undo_move():
    ...
    if _maze_loader and _player:
        if _maze_loader.undo_last_move(_player):
            ...
            # Phạt điểm khi Undo
            _steps_taken = max(0, _steps_taken - 1) 
...
def _draw_hud(screen):
    """Vẽ thông số lên bảng bên trái (chỗ con rắn)"""
    if not _text_designer: return
    
    # Tính thời gian
    elapsed = (pygame.time.get_ticks() - _level_start_time) // 1000
    mins = elapsed // 60
    secs = elapsed % 60
    time_str = f"{mins:02}:{secs:02}"
    
    # Vẽ Time
    # Sử dụng font nhỏ hoặc pyramid font cho số
    lbl_time = _text_designer.render_default(f"TIME: {time_str}")
    screen.blit(lbl_time, (25, 95)) # Vị trí trên đầu con rắn

    # Vẽ Steps
    lbl_steps = _text_designer.render_default(f"MOVES: {_steps_taken}")
    screen.blit(lbl_steps, (25, 115))
        ...
        if _turn_state == TurnState.PLAYER_MOVING:
            if not _player.is_moving:
                ...
                if _maze_loader.check_trap_collision(_player.x, _player.y):
                    ...
                    return None
                if _is_win():
                    # Trả về kết quả thắng kèm stats
                    elapsed = pygame.time.get_ticks() - _level_start_time
                    score = max(0, 1000 - (elapsed // 1000) * 2 - _steps_taken * 5)
                    return ("win", score, elapsed)
                ...
                if now - _death_step_timer > 550:
                    if _killer_ref and "scorpion" in _killer_ref.type:
                        ...
                        if _killer_ref and "mummy" in _killer_ref.type:
                            _player.state = (
                                PlayerState.DIE_RED_MUMMY if _killer_ref.type == "red_mummy" else PlayerState.DIE_WHITE_MUMMY
                            )
    ...
    if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
        ...
        if not _show_options:
            ...
            if clicked_btn == "reset":
                restart_level()
                return "reset"
            # ✅ Fix Quit Button: Returns 'quit' immediately regardless of turn state
            if clicked_btn == "quit":
                return "quit"

        if _show_options:
            # (Logic volume cũ giữ nguyên...)
            pass

    if _show_options: return None
    
    # Chỉ cho di chuyển nếu đang đến lượt người chơi
    if _turn_state == TurnState.PLAYER_INPUT:
